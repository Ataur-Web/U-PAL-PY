...
async def generate(
    *,
    ...
    # strip markdown bold markers (**text** -> text) from LLM responses
    # so they display cleanly in the chat UI without markdown syntax
    before_bold = reply
    reply = re.sub(r"\*\*([^*]+?)\*\*", r"\1", reply)
    if reply != before_bold:
        log.debug("Stripped markdown bold markers from reply")
