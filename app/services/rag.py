...
chromadb stores 4,500+ indexed passages from multiple sources:
- uwtsd scraped corpus (336 verified pages with source urls)
- curated knowledge base (intent-driven student responses)
- welsh terminology bootstrap (academic cy vocabulary)

retrieval strategy:
1. dense pass: multilingual embeddings (paraphrase-MiniLM-L12-v2) for semantic similarity
2. sparse pass: bm25 keyword matching for exact term hits
3. fusion: reciprocal rank fusion combines both rankings into single result list
...
bm25 index is cached in-memory and rebuilt on new ingestions for fast retrieval.
